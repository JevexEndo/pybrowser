...
from pybrowser.bookmark_widget import BookmarkWidget
from pybrowser.browser_tab_widget import BrowserTabWidget
from pybrowser.download_widget import DownloadWidget
from pybrowser.find_toolbar import FindToolBar
from pybrowser.webengine_view import WebEngineView
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.connect(
            self._tab_widget, QtCore.SIGNAL("url_changed(QUrl)"), self.url_changed
        )
    ...
    def _create_menu(self):
        ...
        back_action = QAction(
            QIcon.fromTheme("go-previous", QIcon(style_icons + "left-32.png")),
            "Back",
            self,
        ...
        forward_action = QAction(
            QIcon.fromTheme("go-next", QIcon(style_icons + "right-32.png")),
            "Forward",
            self,
        ...
        reload_action = QAction(
            QIcon(style_icons + "refresh-32.png"),
            "Reload",
            self,
        ...
        new_tab_action = QAction(
            "New Tab", self, shortcut="Ctrl+T", triggered=self.add_browser_tab
        )
        ...
        history_action = QAction(
            "History...", self, triggered=self._tab_widget.show_history
        )
        ...
        add_bookmark_action = QAction(
            "&Add Bookmark", self, triggered=self._add_bookmark
        )
        self._bookmark_menu.addAction(add_bookmark_action)
        add_tool_bar_bookmark_action = QAction(
            "&Add Bookmark to Tool Bar", self, triggered=self._add_tool_bar_bookmark
        )
        ...
        download_action = QAction(
            "Open Downloads", self, triggered=DownloadWidget.open_download_directory
        )
    ...
    def _download_requested(self, item):
        ...
        for old_download in self.statusBar().children():
            if (
                type(old_download).__name__ == "DownloadWidget"
                and old_download.state() != QWebEngineDownloadRequest.DownloadInProgress
            ):

...
class DownloadWidget(QProgressBar):
    ...
    def __init__(self, download_item: QWebEngineDownloadRequest):
        ...
        download_item.isFinishedChanged.connect(self._finished)
        download_item.receivedBytesChanged.connect(self._download_progress)
    ...
    def _update_tool_tip(self):
        path = self._download_item.downloadDirectory()
    ...
    def _download_progress(self):
        if self._download_item.totalBytes() > 0:
            self.setValue(
                int(
                    100
                    * self._download_item.receivedBytes()
                    / self._download_item.totalBytes()
                )
            )
    ...
    def _launch(self):
        DownloadWidget.open_file(self._download_item.downloadDirectory())
    ...
    def contextMenuEvent(self, event):
        ...
        show_in_folder_action.setEnabled(
            state == QWebEngineDownloadRequest.DownloadCompleted
        )
        ...
        if chosen_action == launch_action:
            ...
            path = QFileInfo(self._download_item.downloadDirectory()).absolutePath()
            DownloadWidget.open_file(path)

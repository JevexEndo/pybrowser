...
class BookmarkWidget(QTreeView):
    ...
    def _remove_item(self, item):
        ...
        button = QMessageBox.question(
            self, "Remove", message, QMessageBox.Yes | QMessageBox.No
        )
    ...
    def write_bookmarks(self):
        ...
        if not directory.isDir():
            ...
            if not QDir(directory.absolutePath()).mkpath(directory.fileName()):
                warnings.warn(f"Cannot create {native_dir_path}.", RuntimeWarning)
                return
    ...
    def _read_bookmarks(self):
        bookmark_file_name = os.path.join(
            QDir.toNativeSeparators(_config_dir()), _bookmark_file
        )
